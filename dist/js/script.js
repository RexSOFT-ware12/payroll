...
$(document).ready(function() {
    ...
    /* End of Script */
     ...
     /* Help Table Script Start */
     if ( $('#empHelp').length > 0 ) {
        $('#empHelp').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": baseurl + "ajax/?case=LoadingHelp",
            "columnDefs": [{
                "targets": 0,
                "className": "dt-center"
            }, {
    ...
    if ( $('#help').length > 0 ) {
        ...
        var holi_table = $('#help').DataTable({
            ...
            "columnDefs": [{
                ...
                "data": null,
                "className": "dt-center",
                "defaultContent": '<button class="btn btn-success btn-xs editHelp"><i class="fa fa-edit"></i></button> <button class="btn btn-danger btn-xs deleteHelp"><i class="fa fa-trash"></i></button>'
            }]
        });
        /* End of Script */

        /* Edit Holiday Script Start */
        $('#help tbody').on('click', '.editHelp', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "ajax/?case=GetHelpByID",
                data     : 'id=' + data[0],
                success  : function(result) {
                    if ( result.code == 0 ) {
                        $("#help_id").val(result.result.help_id);
                        $("#help_title").val(result.result.help_title);
                        $("#help_desc").val(result.result.help_desc);
                        $("#help_date").val(result.result.help_date).datepicker('update');
                        $("#help_anwser").val(result.result.help_anwser);
         ...
         /* Delete Holiday Script Start */
         $('#help tbody').on('click', '.deleteHelp', function(e) {
            e.preventDefault();
            ...
            if ( conf ) {
                ...
                $.ajax({
                    ...
                    async    : true,
                    cache    : false,
                    url      : baseurl + "ajax/?case=DeleteHelpByID",
                    data     : 'id=' + data[0],
                    success  : function(result) {
                        if ( result.code == 0 ) {
                            $.notify({
    ...
    if ( $('#help-form').length > 0 ) {
        $('#help-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                async    : true,
                cache    : false,
                url      : baseurl + "ajax/?case=InsertUpdateHelp",
                data     : form.serialize(),
                success  : function(result) {
                    if ( result.code == 0 ) {
                        $.notify({
